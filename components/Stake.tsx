...
export default function Stake() {
  ...
  function validateStakeAmount(amount: string): boolean {
    const minimumStakeAmount = 1;
    const parsedAmount = parseFloat(amount);

    if (isNaN(parsedAmount) || parsedAmount < minimumStakeAmount) {
      setValidationError(`Minimum stake amount is ${minimumStakeAmount}`);
      return false;
    }
  ...
  return (
    <Card p={5} mt={10}>
      <Heading>Earn Reward Token | Current APR 60%</Heading>
      <SimpleGrid minChildWidth={"60"} columns={2}>
        <Card p={5} m={5}>
          ...
          <SimpleGrid minChildWidth={"60"} columns={2} spacing={4}>
            <Stack spacing={4}>
              <Input
                type="number"
                ...
                onChange={(e) => {
                  ...
                  validateStakeAmount(e.target.value);
                }}
              />
              {validationError && (
